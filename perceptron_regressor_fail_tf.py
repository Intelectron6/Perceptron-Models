#Single Layered Perceptron Regressor using Tensorflow to fit a higher degree polynomial
...
Y = 0.67*X + 0.33*(X**2) - 0.5*(X**3) + 0.1*(X**4) 
for i in range(len(Y)):
    Y[i] = Y[i] + 100*np.random.random(1)
...
if (Rsq[0][0] < 75):
    print("Perceptron can't fit a non-linear function")
else:
    print("Fluke! Try again, the R-square value will be less")
